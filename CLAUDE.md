...
| Generating the bitmap stream | **BLOCKED on the encoder, not the protocol.** liblzma accepts the captured stream with its declared size and rejects one built here: their encoder omits the end-of-stream marker, Python's always writes one, and a declared size beside a marker is invalid. Truncating does not help - the marker is entropy-coded, not trailing bytes. Untested as of this note: a plain unknown-size `.lzma` stream *with* the now-correct 8KB dictionary and 48x256 image, which is what `--no-declare-size` sends. If that is refused too, finishing this needs an LZMA encoder that can omit the marker - a new dependency, and a decision to take deliberately rather than a gap to fill. |
