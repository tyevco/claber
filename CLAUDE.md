...
| The T50M Pro's payload protocol | **UNKNOWN.** The transport is settled but the framing inside those 64-byte reports is not, and nothing in this repo drives the device. `mplabel inventory` writes a CSV for SUPVAN's own editor instead. Ground truth would be a USBPcap capture of their editor printing two labels differing in one thing, so the diff separates framing from raster. |
