...
A real `mplabel scan` then came back with 99 messages, every subject
classified, nothing unrecognised - and **not one win and not one payment
receipt**, the two kinds the auction importer exists for. They had been
archived. So the run looked clean, and an import from that account would
have created no inventory and no cost basis at all while reporting
success. `survey_folder` prefers the `\All` special-use mailbox now,
asked for by its RFC 6154 flag rather than by name - Gmail calls it
`[Gmail]/All Mail` in English and something else everywhere else, and a
hardcoded name would have failed the same silent way. A configured
folder that is not the inbox still wins, because setting one is a
deliberate answer to "her mail is filtered into a label". And `scan`
now names the folder it walked: "99 messages" means one thing out of an
inbox and another out of everything she has ever received.
...
but the recognised list held no win and no payment receipt at all, out
...
right, so the body parsing is verified on real mail rather than on a
reconstruction of it. Two things came out of doing that: the fixtures
were missing the `<font color>` interleaving that 4 of the 5 real wins
carry, which is now in them; and a test that the seller's city is told
apart from her home address was passing without telling them apart,
because the fixture used one town for both.
