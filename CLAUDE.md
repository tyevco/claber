...
| Generating the bitmap stream | **The encoder is verified on the hardware; the image is not.** The device wants a declared size and **no end-of-stream marker**, which Python's `lzma` cannot emit - `src/mplabel/lzma1.py` is a literals-only LZMA1 encoder that does. `--reencode` re-encoded a captured image with it and **the label printed**, so a literals-only body is accepted and the transport, the framing and the encoder are all settled. What is still refused is a *generated pattern*, and the two candidates are measured: the print that worked was **0.13% ink in 7 reports**, the one refused **7.54% in 12**. `--style scatter` separates them in one label - working-end ink (0.26%) at failing-end size (695 bytes, 11 reports). Refused means size is the blocker; printed means ink is. Note 419 bytes fits a 512-byte buffer and 724 does not, which is the shape to expect if it is size. |
...
A replayed vendor stream printed, and so did a captured image
**re-encoded by `lzma1.py`** - so the encoder is settled on hardware. What
has never printed is an image this repo *drew*, and the open question is
which property of it the device objects to. See the table above. Until
that lands, nothing renders an inventory label automatically.
